    ...
    async def on_interaction(self,itr):
        if itr.type==discord.InteractionType.application_command and itr.data["name"]=="wipe":
            if itr.data["options"][0]["name"]=="off":
                ...
                await itr response.send_message(f"<:mwipeyay:851572058382925866> Successfully wiped {len(pur)} message{s}." if len(pur)>0 else "<:mno:851569517242351616> No messages were wiped.")
                return
